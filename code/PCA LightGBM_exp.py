...
df = pd.read_excel(r"C:\Users\xzr\Desktop\train_2.xlsx")
...
lgb_reg = lgb.LGBMRegressor(
    boosting_type='gbdt',
    num_leaves=30,              # 控制树的复杂度（比20略大一点）
    max_depth=6,                # 限制最大深度，防止过拟合
    learning_rate=0.02,         # 更小的学习率，配合早停
    n_estimators=2000,          # 增加迭代次数，但配合早停
    objective='regression',
    min_child_samples=40,       # 防止叶子节点过拟合
    min_child_weight=0.001,
    subsample=0.7,              # 引入更多样本随机性
    colsample_bytree=0.7,       # 引入特征随机性
    reg_alpha=0.8,              # L1 正则增强
...
# Feature names
modified_names = [name.replace('heating_rate(℃/min)', 'Heating Rate') 
                 for name in exclude_for_pca]
modified_names = [name.replace('Catalysts_Mass_Fraction(wt%)', 'Cat. Frac.') 
                 for name in modified_names]
retained_feature_names = modified_names
pca_feature_names = [f'PCA{i}' for i in range(X_train_pca.shape[1])]
all_feature_names = retained_feature_names + pca_feature_names
...
selected_features = [
    ...
    'Heating Rate',
    'PCA1', 'PCA2', 'PCA3', 'PCA4',
    'PCA5'  # Add target variable as last feature
]
