...
# 初始化NMF模型并降维到28维
nmf = NMF(n_components=17, random_state=42, max_iter=1000)
...
xgbt = xgb.XGBRegressor(objective='reg:squarederror', learning_rate=0.2, gamma=0.90,
                        ...
                        min_child_weight=15, subsample=1, colsample_bytree=1,n_estimators=900,
                        random_state=42)
...
xgbt = xgb.XGBRegressor(objective='reg:squarederror', learning_rate=0.1, gamma=0.90,
                        reg_alpha=0.9, reg_lambda=0.9, max_depth=3, 
                        min_child_weight=11, subsample=1, colsample_bytree=1,n_estimators=900,
                        random_state=42)
xgbt.fit(X_train_combined, y_train)
...
# 调整布局并保存
plt.subplots_adjust(top=0.95, bottom=0.1, left=0.1, right=0.95)
plt.savefig(r'C:\Users\18387\Desktop\小论文图片\train_test_Residuals_xgbt.png', dpi=640, bbox_inches='tight')
plt.show()
