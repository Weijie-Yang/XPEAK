...
# 定义需要排除的列（不参与NMF的特征）
exclude_for_nmf = ["Catalysts_Mass_Fraction(wt%)", "heating_rate(℃/min)"]

# --- 从原始数据中分离出需要保留的列 ---
# 训练集保留的列
X_train_retained = X_train[exclude_for_nmf].values  # 提取为NumPy数组
# 测试集保留的列
X_test_retained = X_test[exclude_for_nmf].values

# --- 对剩余特征进行NMF降维 ---
# 移除 exclude_for_nmf 列后的特征矩阵
X_train_for_nmf = X_train.drop(exclude_for_nmf, axis=1)
X_test_for_nmf = X_test.drop(exclude_for_nmf, axis=1)

# 初始化NMF模型并降维到86维
nmf = NMF(
    n_components=86,
    init='nndsvda',        # 使用确定性初始化
    random_state=42,       # 固定随机种子
    max_iter=3000,
    tol=1e-6,               # 设置较严格的收敛容差
    solver='cd'            # 默认值，坐标下降法，也可显式指定
)
X_train_nmf = nmf.fit_transform(X_train_for_nmf)  # 训练集拟合+转换
X_test_nmf = nmf.transform(X_test_for_nmf)      # 测试集仅转换

# --- 合并保留列和降维后的特征 ---
# 按列拼接（水平合并）
X_train_combined = np.hstack([X_train_retained, X_train_nmf])
X_test_combined = np.hstack([X_test_retained, X_test_nmf])
...
lgb_reg = lgb.LGBMRegressor(
    ...
    num_leaves=8,              # 每棵树的最大叶子数
    max_depth=-1,               # 树的最大深度（-1表示不限制）
    learning_rate=0.11,         # 学习率
    n_estimators=1000,           # 树的数量
    objective='regression',     # 回归任务
    min_child_samples=11,       # 叶子节点最小样本数
...
# 训练模型
lgb_reg.fit(X_train_combined, y_train)

# 训练集评估（使用X_train_combined）
train_pred = lgb_reg.predict(X_train_combined)
...
test_pred = lgb_reg.predict(X_test_combined)
...
scores = cross_val_score(lgb_reg, X_train_combined, y_train, scoring='neg_mean_squared_error', cv=crossvalidation, n_jobs=-1)
rmse_scores = [np.sqrt(abs(s)) for s in scores]
r2_scores = cross_val_score(lgb_reg, X_train_combined, y_train, scoring='r2', cv=crossvalidation, n_jobs=-1)
mae_scores = cross_val_score(lgb_reg, X_train_combined, y_train, scoring='neg_mean_absolute_error', cv=crossvalidation, n_jobs=-1)
...
modified_names = exclude_for_nmf.copy()
...
# NMF特征名
nmf_feature_names = [f'NMF{i}' for i in range(X_train_nmf.shape[1])]

# 合并所有特征名
all_feature_names = retained_feature_names + nmf_feature_names
...
# Feature names
modified_names = [name.replace('heating_rate(℃/min)', 'Heating Rate') 
                 for name in exclude_for_nmf]
modified_names = [name.replace('Catalysts_Mass_Fraction(wt%)', 'Cat. Frac.') 
                 for name in modified_names]
retained_feature_names = modified_names
nmf_feature_names = [f'NMF{i}' for i in range(X_train_nmf.shape[1])]
all_feature_names = retained_feature_names + nmf_feature_names
...
# 1. Create DataFrame with features AND target variable
features_df = pd.DataFrame(X_train_combined, columns=all_feature_names)
features_df['Tp'] = y_train  # Add target variable

# 2. Select features including Tpeak
selected_features = [
    'Tp',
    'Cat. Frac.', 
    'Heating Rate',
    'NMF1', 'NMF2', 'NMF3', 'NMF4',
    'NMF5'  # Add target variable as last feature
]
data = features_df[selected_features]

# 3. Update feature name list with "Tpeak"
    ...
    for j in range(n):
        ...
        if i == j:
            ...
            sns.histplot(data.iloc[:, i], kde=True, ax=ax, 
                         bins=15,  # 减少bins数量使条形更宽
                         linewidth=6,  # KDE线条宽
                         edgecolor=None,  # 条形边缘颜色
                         color=None,  # 条形填充颜色
                         alpha=0.5)  # 透明度

            # 设置当前子图的x轴范围为该变量的范围
            ax.set_xlim(data.iloc[:, i].min(), data.iloc[:, i].max())
            ...
            sns.regplot(
                ...
                scatter=True,
                ci=95,
                line_kws={"linewidth": 2, "color": "#e84545"},  # 拟合线颜色（红色）
                scatter_kws={
                    "alpha": 0.3, 
                    "s": 20, 
                    "color": None  # 散点颜色改为淡蓝色
                }
            )
            ...
            if j == 0:
                axes[n - 1, j].set_yticks(axes[n - 1, j].get_yticks())
            else:
                ax.set_yticks([])
            ax.set_xlabel('')
            ax.set_ylabel('')
...
# 设置底部和左边的轴标签
for j in range(n):
    axes[n - 1, j].set_xlabel(data.columns[j], fontsize=20, fontweight='bold')
for i in range(n):
    axes[i, 0].set_ylabel(data.columns[i], fontsize=20, fontweight='bold')
...
cbar.set_label('Correlation', fontsize=24, fontweight='bold')  # 标签加粗
...
# 保存并显示图形
plt.savefig(r"C:\Users\xzr\Desktop\paperpic\improved_correlation_matrix_ICDD_NMF_tp.png", 
           format='png', dpi=640, bbox_inches='tight')
plt.show()
...
plt.xlabel('Actual $T_{\t{p}}$(°C)', fontsize=32, labelpad=8)
plt.ylabel('Predicted $T_{\t{p}}$(°C)', fontsize=32, labelpad=8)
...
# 控制刻度样式（字体、大小、加粗、方向）
ax.tick_params(axis='both', which='major', labelsize=30, width=1.5, length=6, direction='out', labelcolor='black')
...
ax_scatter.set_xlabel('Predicted $T_{\t{p}}$(°C)', fontsize=36, fontweight='bold')
...
# 初始化NMF模型并降维到28维
nmf = NMF(n_components=86, random_state=42, max_iter=2000)
X_train_nmf = nmf.fit_transform(X_train_for_nmf)  # 训练集拟合+转换
X_test_nmf = nmf.transform(X_test_for_nmf)      # 测试集仅转换
...
# 1. Create DataFrame with features AND target variable
features_df = pd.DataFrame(X_train_combined, columns=all_feature_names)
features_df['ΔTp'] = y_train  # Add target variable
...
# 保存并显示图形
plt.savefig(r"C:\Users\xzr\Desktop\paperpic\improved_correlation_matrix_ICDD_NMF_Δtp.png", 
           format='png', dpi=640, bbox_inches='tight')
plt.show()
