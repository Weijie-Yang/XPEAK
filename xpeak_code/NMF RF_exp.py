...
# 提取保留列并确保列名是字符串
X_train_retained = X_train[exclude_for_nmf].copy()
X_test_retained = X_test[exclude_for_nmf].copy()
...
X_train_nmf = nmf.fit_transform(X_train_for_nmf)
X_test_nmf = nmf.transform(X_test_for_nmf)

# 为NMF生成的列赋予字符串名称（如 "NMF_0", "NMF_1", ...）
nmf_columns = [f"NMF_{i}" for i in range(17)]

# 合并保留列和NMF列，构建DataFrame并确保列名是字符串
X_train_combined = pd.DataFrame(
    np.hstack([X_train_retained.values, X_train_nmf]),
    columns=exclude_for_nmf + nmf_columns
)

X_test_combined = pd.DataFrame(
    np.hstack([X_test_retained.values, X_test_nmf]),
    columns=exclude_for_nmf + nmf_columns
)
...
# ===== 单样本解释 ===== 
sample_to_explain = X_train_combined.iloc[0].values
plt.figure(figsize=(12, 4))
shap.force_plot(
    explainer.expected_value,
    shap_values[0, :],
    sample_to_explain,  # 使用正确格式的数据
    feature_names=all_feature_names,
    matplotlib=True,
    text_rotation=15,
    show=False
...
try:
    ...
    # 回退检查不同的命名格式
    catalyst_idx = next((i for i, name in enumerate(all_feature_names) 
                         if "cat" in name.lower() or "catalyst" in name.lower()), 0)

try:
    heating_idx = all_feature_names.index('Heating Rate')
except ValueError:
    # 回退检查不同的命名格式
    heating_idx = next((i for i, name in enumerate(all_feature_names) 
                        if "heating" in name.lower() or "rate" in name.lower()), 1)

print(f"催化剂索引: {catalyst_idx}, 加热速率索引: {heating_idx}")
...
rf = RandomForestRegressor(n_estimators=600, random_state=42,max_depth=100,min_samples_split=2,min_samples_leaf=1,criterion='absolute_error',
