...
xgbt = xgb.XGBRegressor(objective='reg:squarederror', learning_rate=0.2, gamma=0.90,
                        reg_alpha=0.9, reg_lambda=0.9, max_depth=3, 
                        min_child_weight=14, subsample=1, colsample_bytree=1,n_estimators=900,
                        random_state=42)
xgbt.fit(X_train_combined, y_train)

# 训练集评估
train_pred = xgbt.predict(X_train_combined)
...
# 测试集评估
test_pred = xgbt.predict(X_test_combined)
...
scores = cross_val_score(xgbt, X_train_combined, y_train, scoring='neg_mean_squared_error', cv=crossvalidation, n_jobs=-1)
rmse_scores = [np.sqrt(abs(s)) for s in scores]
r2_scores = cross_val_score(xgbt, X_train_combined, y_train, scoring='r2', cv=crossvalidation, n_jobs=-1)
mae_scores = cross_val_score(xgbt, X_train_combined, y_train, scoring='neg_mean_absolute_error', cv=crossvalidation, n_jobs=-1)
...
# 检查合并后的维度
print("训练集合并后维度:", X_train_combined.shape)  # 应为 (样本数, 2+28=30)
print("测试集合并后维度:", X_test_combined.shape)   # 同上
...
xgbt = xgb.XGBRegressor(objective='reg:squarederror', learning_rate=0.4, gamma=0.05,
                        reg_alpha=0.97, reg_lambda=0.92, max_depth=5, 
                        min_child_weight=12, subsample=1, colsample_bytree=0.97,n_estimators=800,
                        random_state=42)
xgbt.fit(X_train_combined, y_train)
...
# 1. 过滤掉残差大于30的训练集点
train_filter = np.abs(train_residuals) <= 30
train_pred_filtered = train_pred[train_filter]
train_residuals_filtered = train_residuals[train_filter]

# 2. 过滤掉残差大于30的测试集点（可选，根据需求）
# test_filter = np.abs(test_residuals) <= 30
# test_pred_filtered = test_pred[test_filter]
# test_residuals_filtered = test_residuals[test_filter]

# 创建自定义布局的画布
plt.figure(figsize=(11, 8))
gs = GridSpec(1, 2, width_ratios=[3, 0.8], wspace=0.01)

# 设置颜色方案
train_color = 'skyblue'
test_color = 'lightcoral'
zero_line_color = 'darkred'
grid_color = 'gray'
...
# 只绘制残差小于30的数据点
ax_scatter.scatter(train_pred_filtered, train_residuals_filtered, alpha=0.7, 
                  color=train_color, s=60, label='Train', edgecolors='w', linewidth=0.5)
ax_scatter.scatter(test_pred, test_residuals, alpha=0.7, 
                  color=test_color, s=60, label='Test', edgecolors='w', linewidth=0.5)
...
ax_scatter.tick_params(axis='both', width=2, length=8, colors='#333333')
ax_scatter.set_xlim(-20, max(np.max(train_pred_filtered), np.max(test_pred)) * 1.05)
ax_scatter.set_ylim(-45, 45)
...
# 图例设置保持不变
ax_scatter.legend(frameon=False, edgecolor='#FFFFFF', framealpha=0.9, bbox_to_anchor=(0, 1.01), loc='upper left')

# 2. 残差核密度图
ax_kde = plt.subplot(gs[1])
# 使用过滤后的数据创建核密度图
sns.kdeplot(y=train_residuals_filtered, fill=True, color=train_color, 
           alpha=0.6, linewidth=2.5, ax=ax_kde)
sns.kdeplot(y=test_residuals, fill=True, color=test_color, 
           alpha=0.4, linewidth=2.5, ax=ax_kde)
ax_kde.axhline(y=0, color=zero_line_color, linestyle='--', linewidth=2)

ax_kde.set_ylim(-45, 45)
ax_kde.axis('off')
